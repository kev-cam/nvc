...
def resolve_net(nets, design_name):
    ...
    # Generate wrapper file — only when there are resolver entities to
    # instantiate.  When the kernel took every net the wrapper is pure
    # noise, and its compilation failure (nvc may not be on PATH) would
    # pollute the simulation output
    if not entity_names:
