...
entity q_lt64 is port (clk, rst_l : in std_logic; y : out std_logic_vector(31 downto 0)); end entity;
architecture rtl of q_lt64 is
  ...
  process (clk) is variable mix : unsigned(31 downto 0); begin
    if rising_edge(clk) then
      if rst_l = '0' then acc <= (others=>'0'); shamt<=0;
        ...
        if shamt >= 40 then shamt <= 0; else shamt <= shamt + 1; end if;
      end if;
    end if;
  end process;
